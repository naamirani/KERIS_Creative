...
public class ImgView {
    ...
    public Scene getScene(String result, File select, CheckBox check,
        ...
        if (!csvPath.equals("Load neurons from CSV file...")) {
            readFromCSV(csvPath, map, result,customStatuses);
        }
        ...
        currNeuronIdList = FXCollections.observableArrayList(currTimePoint.neurons.keySet());
        currStatuses = currTimePoint.neuronStatus;
        if(currNeuronIdList == null){
            currNeuronIdList = FXCollections.observableArrayList();
        }
        ...
        String fn;
        if(isWindows()) {
            fn = outputDir+"\\"+currWellList.get(currIndex);;
        }
        else {
            fn = outputDir+"/"+currWellList.get(currIndex);
        }
                ...
                if (res.isPresent()) {
                    if (res.get().equals(ButtonType.YES)) {
                        if (timePoints.indexOf(currTimePoint) == timePoints.size()-1) {
                            ...
                            String ssPath;
                            if (isWindows()) {
                                ssPath = inputDir + "\\screenshot-" + currWellName + "-T" + currTimePoint.timepointNum + ".png";
                            } else {
                                ssPath = inputDir + "/screenshot-" + currWellName + "-T" + currTimePoint.timepointNum + ".png";
                            }
                ...
                e -> {
                    if (e.getButton() == MouseButton.PRIMARY) {
                        if (customStatuses || (!customStatuses && (!currStatuses.containsKey(currTimePoint.currLabelIndex) || currStatuses.get(currTimePoint.currLabelIndex).get(0).equals("Alive")))) {
                            ...
                            // ELSE
                            neuron.setLastTp(Integer.toString(currTimePoint.tpIndex));
...
//                currTimePoint.removeAll();
        ...
        EventHandler<ActionEvent> homePressEvent = new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent event) {
                ...
                String ssPath;
                if (isWindows()) {
                    ssPath = inputDir + "\\screenshot-" + currWellName + "-" + currTimePoint.timepointNum + ".png";
                } else {
                    ssPath = inputDir + "/screenshot-" + currWellName + "-" + currTimePoint.timepointNum + ".png";
                }
                File ss = new File(ssPath);
        ...
        EventHandler<MouseEvent> updateStatusListView = new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                if (selectedStatus != null) {
                    ...
                    if(currTimePoint.getNeurons().containsKey(selectedNeuron)) {
                    ...
                    if (!customStatuses && (selectedStatus.get(0).equals("Dead") || selectedStatus.get(0).equals("Censored"))) {
                        // SET NEURON LAST TP
                        currTimePoint.getNeurons().get(selectedNeuron).setLastTp(Integer.toString(currTimePoint.tpIndex));
                        viewPane.getChildren().remove(currTimePoint.getMarkersGroup());
                        viewPane.getChildren().remove(currTimePoint.getLabelsGroup());
...
//                        String oldStatuses = oldRow.get(5);
//                        String currStatus = currNeuron.status.toString();
//                        String newStatus = oldStatuses.replace("]", "\\" + currStatus + "]");
//
//
...
//
//                        String oldStatuses = oldRow.get(5);
//                        oldStatuses = oldStatuses.replace("]", "\\-]");
//                        newRow.add(oldStatuses);
//
//
//                        int isCensored = 1;
...
//        }
    ...
    public void printCSVCustom(File select, String outputDir){
        ...
        HashMap<Integer, ArrayList<String>> rows = new HashMap<Integer, ArrayList<String>>();
        int tempIndex = 0;
        ArrayList<Integer> numNeuron = new ArrayList<Integer>();
        for(String tempWellName : map.keySet()) {
            int numNeurons =0;
            LinkedList<Timepoint> currtimePoints = wellTimepoints.get(tempWellName);
            for(int i= 0;i<currtimePoints.size();i++){
                if(currtimePoints.get(i).getNeurons().size()>numNeurons){
                    numNeurons = currtimePoints.get(i).getNeurons().size();
                }
            }
            numNeuron.add(numNeurons);
            for (int i = tempIndex; i < tempIndex+numNeurons; i++) {
                ArrayList<String> row = new ArrayList<String>();
                row.add(tempWellName);
                row.add(Integer.toString(i-tempIndex+1));
        ...
        for(String wellName : map.keySet()){
            ...
            for (int i = 0; i < tempTimePoints.size(); i++) {
                HashMap<Integer, Neuron> neurons = tempTimePoints.get(i).getNeurons();
                for (int index = 1;index<=numNeuron.get(j);index++) {
                    if(neurons.containsKey(index)) {
                        ArrayList<String> newRow = new ArrayList<String>();
                        ArrayList<String> oldRow = rows.get(rowIndex+index-1);
                        newRow.add(oldRow.get(0));
                        ...
                        newRow.add(y);
                        String oldStatuses = oldRow.get(5);
                        if(currNeuron.status.size()==0){
                            oldStatuses = oldStatuses.replace("]", "\\-]");
                        }
                        else{
                            String statuses = currNeuron.status.toString().replace("[","{");
                            statuses = statuses.replace("]","}");
                            statuses = statuses.replace(",","");
                            oldStatuses = oldStatuses.replace("]", "\\" + statuses + "]");
                        }
                        newRow.add(oldStatuses);
                        rows.put(rowIndex+index-1, newRow);
                    }
            ...
            rowIndex = rowIndex + numNeuron.get(j);
            j++;
        }
        FileWriter csvWriter;
        try {
    ...
    public void readFromCSV(String inputPath, Map<String, List<String>> map, String result,boolean isCustom) throws FileNotFoundException {
        //Parse CSV
        ...
        if(shouldReadCSV) {
            ...
            for (int i = 1; i < storedNeurons.size(); i++) {
                ...
                List<String> lifeStatus =  Arrays.asList(storedNeurons.get(i).get(5).split(Pattern.quote(separator)));
                int currIndex = 1;
                ...
                for (int j = 0; j < tempTimePoints.size(); j++) {
                    ...
                    String curr = lifeStatus.get(currIndex).replace("{","");
                    curr = curr.replace("}","");
                    curr = curr.replace("]", "");
                    List<String> currStatuses = Arrays.asList(curr.split(" "));
                    if(currStatuses.get(0).equals("-")){
                        currStatuses = new ArrayList<String>();
                    }
                    if (!xString.equals("-")) {
                        ...
                        HashMap<Integer, List<String>> currNeuronStatuses = tempTimePoints.get(j).neuronStatus;
                        currNeuronStatuses.put(labelIndex, currStatuses);
                        tempTimePoints.get(j).add(neuron, currNeuronStatuses, labelIndex, result,customStatuses);
                        //generate listview for neurons for censor
                        //ObservableList<Integer> tempNeuronIdList = FXCollections.observableArrayList(tempTimePoints.get(j).neurons.keySet());
                    }
                    currIndex++;
                }
